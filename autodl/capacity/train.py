"""
Training loop with per-group capacity penalty.
Adds --capacity/--cap flags. 10 termination conditions (see arch/train.py).
"""
import argparse
import json
import math
...
def main():
    ...
    ap.add_argument("--capacity", type=float, default=0.0,
                    help="per-group capacity penalty weight. >0 = run slope_k mechanism experiment; sweep for direction, do not tune to target")
    ap.add_argument("--cap", type=float, default=C.CAPACITY,
                    help="soft per-group capacity (points), default 4 ~ working-memory capacity")
    ap.add_argument("--max-hours", type=float, default=C.MAX_HOURS)
    ap.add_argument("--no-amp", action="store_true")
    ap.add_argument("--resume", action="store_true")
    a = ap.parse_args()
    C.W_REG_CAP = a.capacity
    C.CAPACITY = a.cap
