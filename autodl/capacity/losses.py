"""
Loss functions (capacity variant).
Primary: pairwise co-assignment BCE. Adds reg_capacity: per-group occupancy
penalty. Lambda swept over dose-response series.
"""
import torch
...
def reg_capacity(P, mask, cap):
    ...
    occ = (P * m).sum(1)
    over = torch.relu(occ - cap) ** 2              # (B,K)
    npts = mask.float().sum(1).clamp(min=1)        # (B,)
    return (over.sum(1) / npts).mean()
...
def total_loss(P, x, y, mask, cfg=C):
    ...
    if cfg.W_REG_CAP:
        parts["reg_cap"] = cfg.W_REG_CAP * reg_capacity(P, mask, cfg.CAPACITY)
